# RScript_CASTAWAY_QC_v11
# written by F.H.Page
# date work on this version started: 2017 May 21
...
# -----------------------------------------------------------------------------
#
# ---- FUNCTION to count the number of header lines in the input CASY+TAWAY file
#

NumHeaderLines <- function(file){
                    n <- 0                           # initialize the counter within the loop
                    header <- "%"                    # initialize the counter within the loop
                    repeat {
                           header <- substr(scan(file, skip=n,  nlines=1, sep="\t", what="character"),0,1)[1]
                           if(header == "%"){
                              n <- n+1
                              } else {                              
                               break
                    ...
                    n <- n-1                         # the last heder line is a blank line
                    }                                # end of function
...
# ------------------------------------------------------------
#
# Notes on File Types: from CASTAWAY Users Manual 1.5 
#                                for Software version 1.5 and
#                                    Firmware version 1.5
# -------------------
# RAW Data
#     This file type contains all unedited and unprocessed data from the cast. 
#     The data includes data prior to entering the water, the data while the 
#     instrument is being held near the surface for thermal equilibration and 
#     flushing of bubbles from the conductivity cell, downcast data, data while 
#     at the bottom of the cast and waiting to to be recovered, upcast data, and
#     data in the air adfter the instrument has been recovered.    
#     
# DOWN CAST
#     This file type only contains data from the down cast. The determination of 
#     what is downcast data is made by the CASTAWAY software.  
# UP CAST
#     This file type only contains data from the up cast. The determination of 
#     what is upcast data is made by the CASTAWAY software. 
# DOWN and UP CAST
#     This file type only contains data from the down and up cast. The determination of 
#     what is downcast and upcast data is made by the CASTAWAY software. 
#     
# PROCESSED CAST
#     This file type only contains data from the down and up cast that are combined 
#     by the CASTAWAY software to produce what CASTAWAY considers the best measurement 
#     possible. The determination of what is downcast and up data and how they are 
#     combined is made by the CASTAWAY software. 
# 
...
cat("        Note: The R,D,U,DU,P designations in the file names \n")
cat("              are a convention adopted by the user. The CASTAWAY \n")
cat("              software that generates these output files does not \n")
cat("              automatically indicate the processing type in the file name. \n")
cat("              The CASTAWAY software records the processing type in the header \n")
cat("              lines contained within it's output file. \n")
cat("\n")
...
cat("        CCxxx...xxP.txt  should contain the CASTAWAY Generated PROCESSED CAST   data \n")
...
# ------------------------------------------------------
# Count the number of header lines in the CASTAWAY file
# ------------------------------------------------------

  NumHeaders <- NumHeaderLines(INPUT.DATA.FILE.NAME)
  cat(" \n")
  cat("The number of header lines in the CASTAWAY file is: ", NumHeaders, "\n")
  cat(" \n")
  if(NumHeaders == 27) {
      cat("The number of header lines is as expected. \n")
      cat("\n")
      option_Y_N <- readline("PRESS ENTER to continue ")
     } else {
      cat("The number of header lines is NOT as expected. \n")
      cat("\n")
      option_CcSs <- readline("ENTER C or c to CONTINUE and S or s to EXIT Program: ")
      if(option_CcSs == "s") {option_CcSs <- "S"}
      if(option_CcSs == "S") {stop("Script has been terminated")}
     } # end of if else
...
# ----------------------------------------------------------------------------------
#
# Each header line in a CASTAWAY output file begins with a percent symbol (%), the comment symbol in MATLAB.
# Each CASTAWAY generated output file is expected to include the following header information.
# The order of the header information is as listed below.
#     Note: The specific information in the following lines is from a specific cast and is meant to be an example only
#
#1  % Device	CC1138020			
#2  % File name	CC1138020_20150801_170916			
#3  % Cast time (UTC)	01/08/2015 17:09			
#4  % Cast time (local)	01/08/2015 17:09			
#5  % Sample type	Cast			
#                 Note: alternatives are Cast and Point
#6  % Cast data	Processed
#                 Note: alternatives are RAW, DOWN, UP, DOWN and UP, PROCESSED			
#7  % Location source	GPS			
#8  % Default latitude	32			
#9  % Default altitude	0			
#10 % Start latitude	45.19989			
#11 % Start longitude	-65.9896599			
#12 % Start altitude	-7.460000038			
#13 % Start GPS horizontal error(Meter)	1.379999995			
#14 % Start GPS vertical error(Meter)	1.669999957			
#15 % Start GPS number of satellites	10			
#16 % End latitude	45.1998191			
#17 % End longitude	-65.990245			
#18 % End altitude	-4.960000038			
#19 % End GPS horizontal error(Meter)	1.730000019			
#20 % End GPS vertical error(Meter)	2.299999952			
#21 % End GPS number of satellites	9			
#22 % Cast duration (Seconds)	90.6			
#23 % Samples per second	5			
#24 % Electronics calibration date	0001-01-01			
#25 % Conductivity calibration date	30/09/2011			
#26 % Temperature calibration date	30/09/2011			
#27 % Pressure calibration date	30/09/2011	

INPUT.Device           <- scan(INPUT.DATA.FILE.NAME, skip=0,  nlines=1, sep="\t", what="character")
...
INPUT.CastTimeLocal    <- scan(INPUT.DATA.FILE.NAME, skip=3,  nlines=1, sep="\t", what="character")
INPUT.SampleType       <- scan(INPUT.DATA.FILE.NAME, skip=4,  nlines=1, sep="\t", what="character")
INPUT.ProcessingType   <- scan(INPUT.DATA.FILE.NAME, skip=5,  nlines=1, sep="\t", what="character")
INPUT.LocSource        <- scan(INPUT.DATA.FILE.NAME, skip=6,  nlines=1, sep="\t", what="character")
INPUT.DefaultLat       <- scan(INPUT.DATA.FILE.NAME, skip=7,  nlines=1, sep="\t", what="character")
INPUT.DefaultAlt       <- scan(INPUT.DATA.FILE.NAME, skip=8,  nlines=1, sep="\t", what="character")
...
INPUT.StartAlt         <- scan(INPUT.DATA.FILE.NAME, skip=11, nlines=1, sep="\t", what="character")
INPUT.StartGPShorErr_m <- scan(INPUT.DATA.FILE.NAME, skip=12, nlines=1, sep="\t", what="character")
INPUT.StartGPSverErr_m <- scan(INPUT.DATA.FILE.NAME, skip=13, nlines=1, sep="\t", what="character")
INPUT.StartGPSnumSat   <- scan(INPUT.DATA.FILE.NAME, skip=14, nlines=1, sep="\t", what="character")
...
INPUT.EndAlt_m	     <- scan(INPUT.DATA.FILE.NAME, skip=17, nlines=1, sep="\t", what="character")
INPUT.EndGPShorErr_m   <- scan(INPUT.DATA.FILE.NAME, skip=18, nlines=1, sep="\t", what="character")
INPUT.EndGPSverErr_m   <- scan(INPUT.DATA.FILE.NAME, skip=19, nlines=1, sep="\t", what="character")
INPUT.EndGPSnumSat     <- scan(INPUT.DATA.FILE.NAME, skip=20, nlines=1, sep="\t", what="character")
INPUT.CastDurSec	     <- scan(INPUT.DATA.FILE.NAME, skip=21, nlines=1, sep="\t", what="character")
INPUT.SampPerSec	     <- scan(INPUT.DATA.FILE.NAME, skip=22, nlines=1, sep="\t", what="character")
INPUT.ElecCalDate	     <- scan(INPUT.DATA.FILE.NAME, skip=23, nlines=1, sep="\t", what="character")
INPUT.CondCalDate	     <- scan(INPUT.DATA.FILE.NAME, skip=24, nlines=1, sep="\t", what="character")
INPUT.TempCalDate	     <- scan(INPUT.DATA.FILE.NAME, skip=25, nlines=1, sep="\t", what="character")
INPUT.PresCalDate      <- scan(INPUT.DATA.FILE.NAME, skip=26, nlines=1, sep="\t", what="character")

# -------------------------------------------
# test each header line for expected content
# -------------------------------------------

if(noquote(INPUT.Device[1]) == "% Device") {
   cat("1st header line is AS EXPECTED i.e contains CASTAWAY Device Number \n")
   } else {
   cat(" \n")
   cat("WARNING: 1st header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Device Number \n")
   } # end of if else

if(noquote(INPUT.FileName[1]) == "% File name") {
   cat("2nd header line is AS EXPECTED i.e contains CASTAWAY File name \n")
   } else {
   cat(" \n")
   cat("WARNING: 2nd header line is NOT AS EXPECTED i.e. does not contain CASTAWAY File name \n")
   } # end of if else

if(noquote(INPUT.CastTimeUTC[1]) == "% Cast time (UTC)") {
   cat("3rd header line is AS EXPECTED i.e contains CASTAWAY Cast time (UTC) \n")
   } else {
   cat(" \n")
   cat("WARNING: 3rd header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Cast time (UTC) \n")
   } # end of if else

if(noquote(INPUT.CastTimeLocal[1]) == "% Cast time (local)") {
   cat("4th header line is AS EXPECTED i.e contains CASTAWAY Cast time (local) \n")
   } else {
   cat(" \n")
   cat("WARNING: 4th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Cast time (local) \n")
   } # end of if else

if(noquote(INPUT.SampleType[1]) == "% Sample type") {
   cat("5th header line is AS EXPECTED i.e contains CASTAWAY Sample type \n")
   } else {
   cat(" \n")
   cat("WARNING: 5th header is line NOT AS EXPECTED i.e. does not contain CASTAWAY Sample type \n")
   } # end of if else

if(noquote(INPUT.ProcessingType[1]) == "% Cast data") {
   cat("6th header line is AS EXPECTED i.e contains CASTAWAY Cast data \n")
   } else {
   cat(" \n")
   cat("WARNING: 6th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Cast data \n")
   } # end of if else

if(noquote(INPUT.LocSource[1]) == "% Location source") {
   cat("7th header line is AS EXPECTED i.e contains CASTAWAY Location source \n")
   } else {
   cat(" \n")
   cat("WARNING: 7th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Location source \n")
   } # end of if else

if(noquote(INPUT.DefaultLat[1]) == "% Default latitude") {
   cat("8th header line is AS EXPECTED i.e contains CASTAWAY Default latitude \n")
   } else {
   cat(" \n")
   cat("WARNING: 8th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Default latitude \n")
   } # end of if else

if(noquote(INPUT.DefaultAlt[1]) == "% Default altitude") {
   cat("9th header line is AS EXPECTED i.e contains CASTAWAY Default altitude \n")
   } else {
   cat(" \n")
   cat("WARNING: 9th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Default altitude \n")
   } # end of if else

if(noquote(INPUT.StartLat[1]) == "% Start latitude") {
   cat("10th header line is AS EXPECTED i.e contains CASTAWAY Start latitude \n")
   } else {
   cat(" \n")
   cat("WARNING: 10th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Start latitude \n")
   } # end of if else

if(noquote(INPUT.StartLon[1]) == "% Start longitude") {
   cat("11th header line is AS EXPECTED i.e contains CASTAWAY Start longitude \n")
   } else {
   cat(" \n")
   cat("WARNING: 11th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Start longitude \n")
   } # end of if else

if(noquote(INPUT.StartAlt[1]) == "% Start altitude") {
   cat("12th header line is AS EXPECTED i.e contains CASTAWAY Start altitude \n")
   } else {
   cat(" \n")
   cat("WARNING: 12th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Start altitude \n")
   } # end of if else

if(noquote(INPUT.StartGPShorErr_m[1]) == "% Start GPS horizontal error(Meter)") {
   cat("13th header line is AS EXPECTED i.e contains CASTAWAY Start GPS horizontal error(Meter) \n")
   } else {
   cat(" \n")
   cat("WARNING: 13th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Start GPS horizontal error(Meter) \n")
   } # end of if else

if(noquote(INPUT.StartGPSverErr_m[1]) == "% Start GPS vertical error(Meter)") {
   cat("14th header line is AS EXPECTED i.e contains CASTAWAY Start GPS vertical error(Meter) \n")
   } else {
   cat(" \n")
   cat("WARNING: 14th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Start GPS vertical error(Meter) \n")
   } # end of if else

if(noquote(INPUT.StartGPSnumSat[1]) == "% Start GPS number of satellites") {
   cat("15th header line is AS EXPECTED i.e contains CASTAWAY Start GPS number of satellites \n")
   } else {
   cat(" \n")
   cat("WARNING: 15th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Start GPS number of satellites \n")
   } # end of if else

if(noquote(INPUT.EndLat[1]) == "% End latitude") {
   cat("16th header line is AS EXPECTED i.e contains CASTAWAY End latitude \n")
   } else {
   cat(" \n")
   cat("WARNING: 16th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY End latitude \n")
   } # end of if else

if(noquote(INPUT.EndLon[1]) == "% End longitude") {
   cat("17th header line is AS EXPECTED i.e contains CASTAWAY End longitude\n")
   } else {
   cat(" \n")
   cat("WARNING: 17th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY End longitude\n")
   } # end of if else

if(noquote(INPUT.EndAlt_m[1]) == "% End altitude") {
   cat("18th header line is AS EXPECTED i.e contains CASTAWAY End altitude \n")
   } else {
   cat(" \n")
   cat("WARNING: 18th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY End altitude \n")
   } # end of if else

if(noquote(INPUT.EndGPShorErr_m[1]) == "% End GPS horizontal error(Meter)") {
   cat("19th header line is AS EXPECTED i.e contains CASTAWAY End GPS horizontal error(Meter) \n")
   } else {
   cat(" \n")
   cat("WARNING: 19th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY End GPS horizontal error(Meter) \n")
   } # end of if else

if(noquote(INPUT.EndGPSverErr_m[1]) == "% End GPS vertical error(Meter)") {
   cat("20th header line is AS EXPECTED i.e contains CASTAWAY End GPS vertical error(Meter) \n")
   } else {
   cat(" \n")
   cat("WARNING: 20th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY End GPS vertical error(Meter) \n")
   } # end of if else

if(noquote(INPUT.EndGPSnumSat[1]) == "% End GPS number of satellites") {
   cat("21st header line is AS EXPECTED i.e contains CASTAWAY End GPS number of satellites \n")
   } else {
   cat(" \n")
   cat("WARNING: 21st header line is NOT AS EXPECTED i.e. does not contain CASTAWAY End GPS number of satellites \n")
   } # end of if else

if(noquote(INPUT.CastDurSec[1]) == "% Cast duration (Seconds)") {
   cat("22nd header line is AS EXPECTED i.e contains CASTAWAY Cast duration (Seconds) \n")
   } else {
   cat(" \n")
   cat("WARNING: 22nd header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Cast duration (Seconds) \n")
   } # end of if else

if(noquote(INPUT.SampPerSec[1]) == "% Samples per second") {
   cat("23rd header line is AS EXPECTED i.e contains CASTAWAY Samples per second \n")
   } else {
   cat(" \n")
   cat("WARNING: 23rd header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Samples per second \n")
   } # end of if else

if(noquote(INPUT.ElecCalDate[1]) == "% Electronics calibration date") {
   cat("24th header line is AS EXPECTED i.e contains CASTAWAY Electronics calibration date \n")
   } else {
   cat(" \n")
   cat("WARNING: 24th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Electronics calibration date \n")
   } # end of if else

if(noquote(INPUT.CondCalDate[1]) == "% Conductivity calibration date") {
   cat("25th header line is AS EXPECTED i.e contains CASTAWAY Conductivity calibration date \n")
   } else {
   cat(" \n")
   cat("WARNING: 25th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Conductivity calibration date \n")
   } # end of if else

if(noquote(INPUT.TempCalDate[1]) == "% Temperature calibration date") {
   cat("26th header line is AS EXPECTED i.e contains CASTAWAY Temperature calibration date \n")
   } else {
   cat(" \n")
   cat("WARNING: 26th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Temperature calibration date \n")
   } # end of if else

if(noquote(INPUT.PresCalDate[1]) == "% Pressure calibration date") {
   cat("27th header line is AS EXPECTED i.e contains CASTAWAY Pressure calibration date \n")
   } else {
   cat(" \n")
   cat("WARNING: 27th header line is NOT AS EXPECTED i.e. does not contain CASTAWAY Pressure calibration date \n")
   } # end of if else

# ---------------------------------------
# strip extra information out of headers
# ---------------------------------------

INPUT.Device           <- noquote(INPUT.Device[2])
INPUT.FileName         <- noquote(INPUT.FileName[2])
INPUT.CastTimeUTC      <- noquote(INPUT.CastTimeUTC[2])
INPUT.CastTimeLocal    <- noquote(INPUT.CastTimeLocal[2])
INPUT.SampleType       <- noquote(INPUT.SampleType[2])
INPUT.ProcessingType   <- noquote(INPUT.ProcessingType[2])
INPUT.LocSource        <- noquote(INPUT.LocSource[2])
INPUT.DefaultLat       <- noquote(INPUT.DefaultLat[2])
INPUT.DefaultAlt       <- noquote(INPUT.DefaultAlt[2])
INPUT.StartLat         <- noquote(INPUT.StartLat[2])
INPUT.StartLon         <- noquote(INPUT.StartLon[2])
INPUT.StartAlt         <- noquote(INPUT.StartAlt[2])
INPUT.StartGPShorErr_m <- noquote(INPUT.StartGPShorErr_m[2])
INPUT.StartGPSverErr_m <- noquote(INPUT.StartGPSverErr_m[2])
INPUT.StartGPSnumSat   <- noquote(INPUT.StartGPSnumSat[2])
INPUT.EndLat           <- noquote(INPUT.EndLat[2])
INPUT.EndLon           <- noquote(INPUT.EndLon[2])
INPUT.EndAlt_m         <- noquote(INPUT.EndAlt_m[2])
INPUT.EndGPShorErr_m   <- noquote(INPUT.EndGPShorErr_m[2])
INPUT.EndGPSverErr_m   <- noquote(INPUT.EndGPSverErr_m[2])
INPUT.EndGPSnumSat     <- noquote(INPUT.EndGPSnumSat[2])
INPUT.CastDurSec       <- noquote(INPUT.CastDurSec[2])
INPUT.SampPerSec       <- noquote(INPUT.SampPerSec[2])
INPUT.ElecCalDate      <- noquote(INPUT.ElecCalDate[2])
INPUT.CondCalDate      <- noquote(INPUT.CondCalDate[2])
INPUT.TempCalDate      <- noquote(INPUT.TempCalDate[2])
INPUT.PresCalDate      <- noquote(INPUT.PresCalDate[2])
...
INPUT.DATA.FILE.NAME    <- substr(INPUT.DATA.FILE.NAME,1,nchar(INPUT.DATA.FILE.NAME)-4)
...
# ----        Note: skip the headers & column label header line        ----
# -------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# ---- Conduct Range checks on the RAW pressure, temperature and conductivity
# -----------------------------------------------------------------------------
